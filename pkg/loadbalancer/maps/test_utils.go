...
func DumpLBMaps(lbmaps LBMaps, sanitizeIDs bool, customizeAddr func(types.AddrCluster, uint16) string) (out []MapDump) {
	...
	stringFromBackendID := func(svcValue ServiceValue, slot int) string {
		if slot != 0 {
	...
	affCB := func(affKey *AffinityMatchKey, _ *AffinityMatchValue) {
	...
	srcRangeCB := func(key SourceRangeKey, _ *SourceRangeValue) {
		key = key.ToHost()

...
func TestGetDeltaResources(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:               "forced missing named resource is removed",
			subscriptions:      []string{"a4"},
			lastAckedVersion:   5,
			forceResponseNames: []string{"a4"},
			wantNames:          []string{},
			wantRemoved:        []string{"a4"},
		},
		{
			desc:               "forced missing resource with wildcard is removed",
			subscriptions:      []string{"*"},
			lastAckedVersion:   5,
			forceResponseNames: []string{"a4"},
			wantNames:          []string{},
		...
		{
			desc:               "wildcard sentinel is not a removed resource",
			subscriptions:      []string{"*"},
			lastAckedVersion:   5,
			forceResponseNames: []string{"*"},
			wantNames:          []string{"a1", "a2", "a3"},

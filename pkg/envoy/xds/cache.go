...
func (c *Cache) GetDeltaResources(typeURL string, lastAckedVersion uint64, subscriptions set.Set[string], ackedResourceNames set.Set[string], forceResponseNames set.Set[string], forceEmptyResponse bool) *VersionedResources {
	...
	var removedNames set.Set[string]
	insertRemovedIfSubscribed := func(name string) {
		// Wildcard name "*" is never present as a resource name
		if name != "*" && (wildcard || subscriptions.Has(name)) {
			key.resourceName = name
	...
	for name := range ackedResourceNames.Members() {
		insertRemovedIfSubscribed(name)
	}
	// A forced missing resource resolves both a new named subscription and a
	// named unsubscription that remains covered by a wildcard subscription.
	for name := range forceResponseNames.Members() {
		if name != "*" {
			insertRemovedIfSubscribed(name)
		}
	}
	res.RemovedNames = removedNames.AsSlice()
